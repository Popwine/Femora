...
struct SparseMatrixCSR {
    ...
    std::vector<size_t> col_indices;  // 非零元素对应的列索引 (通常用 int 或 size_t)
    std::vector<size_t> row_ptr;      // 行指针 (大小为 num_rows + 1)
};
...
std::vector<real> spmv_csr(const SparseMatrixCSR& A, const std::vector<real>& x){
    if(x.size() != A.num_cols){
        throw std::runtime_error("Error in spmv_csr: The number of A's colums doesn't match x's length.");
    }
    std::vector<real> result;
    for(size_t i = 0; i < A.num_rows; i++){
        size_t current_ptr = A.row_ptr[i];
        real sum = 0.0;
        
        while(true){
            real A_value = A.values[current_ptr];
